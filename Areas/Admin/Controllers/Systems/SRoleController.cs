...
namespace ShopAdmin.Areas.Admin.Controllers.Systems
{
    ...
    public class SRoleController : BaseController
    {
        ...
        public async Task<JsonResult> Create(SRole item, string strPermissionId)
        {
            HttpMessage httpMessage = new HttpMessage(true);
            using (var trans = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var newItem = await db.SRoles.AddAsync(item);
                    ...
                    if (lstPermissionId.Any())
                    {
                        ...
                        {
                            RoleId = newItem.Entity.Id,
                            PermissionId = x,
                            IsGranted = 1
                        }).ToList();
                        await db.SRolePermissions.AddRangeAsync(lstPermissionRole);
                        await db.SaveChangesAsync();
                    }
