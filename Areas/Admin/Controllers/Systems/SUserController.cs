...
namespace ShopAdmin.Areas.Admin.Controllers.Systems
{
    ...
    public class SUserController : BaseController
    {
        ...
        public async Task<JsonResult> GetTable(SearchDto itemSearch)
        {
            ...
            try
            {
                var query = db.SUsers.AsNoTracking();
                ...
                var result = await query.Select(x => new SUserViewDto { Id = x.Id, Username = x.Username, HoTen = x.HoTen, Email = x.Email, ChucVu = x.ChucVu.Value, IsActived = x.IsActived }).OrderBy(x => x.Id).Skip(itemSearch.Skip).Take(itemSearch.PageSize).ToListAsync().ConfigureAwait(false);
