var $header = $('.frame-search');
const $footer = $('.frame-footer');
const $table = $('#TBLDANHSACH');
const $tableBody = $table.find('tbody');
const $modal = $('#CHITIET');
const $router = "Admin/Order";
const $form = $('#ModalForm');


function DataSearch(pageNum) {
    this.Ten = $header.find('[name=Ten]').val().trim();
    this.TrangThai = $header.find('[name=TrangThai]').val();
    this.TrangThaiThanhToan = $header.find('[name=TrangThaiThanhToan]').val();
    this.PageNum = pageNum || $footer.find('[name=PageNumber]').val();
    this.PageSize = $footer.find('[name=PageLength]').val();
}

$(function () {
    var $page = {
        init: function () {
            $page.initValidate();
            $page.BtnSearchClick();
            $page.GetList(1);
        },
        Self: $('.card'),
        initValidate: function () {
            $form.validate({
                rules: {
                    Status: {
                        required: true
                    }
                },
                messages: {
                    Status: {
                        required: "Vui lòng chọn trạng thái"
                    }
                }
            })
        },
        BtnSearchClick: function () {
            $header.find('.btnSearch').on('click', function () { $page.GetList(1); });
        },
        GetList: function (pageNum = 1) {

            let html = '';
            let search = new DataSearch(pageNum);

            var getResponse = AjaxConfigHelper.SendRequestToServer(`/${$router}/GetTable`, "GET", search);
            getResponse.then((res) => {
                if (res.IsOk) {
                    let data = res.Body.Data || [];

                    if (data.length == 0) {
                        html = `<tr><td class="text-center" colspan="9"><span>Không có bản ghi</span></td></tr>`;
                        $tableBody.html(html);
                        return false;
                    }

                    let startIndex = res.Body.Pagination.StartIndex || 1;
                    for (let item of data) {
                        //let htmlShow = item.Enable == 1 ? '<span class="text-center green"><i class="text-center green icon-status-ico-tsd"></i></span>' : '<span class="text-center red"><i class="text-center red icon-exit-ico-tsd"></i></span>';
                        html += `<tr class="TR_${item.Id}">` +
                            `<td class="text-center"><span>${startIndex}</span></td>` +
                            `<td class="text-center event-handle">` +
                            `<i class="icon-edit-ico-tsd btn-action btnEdit blue mr10" data-id="${item.Id}" title="Sửa"></i>` +
                            `</td>` +
                            `<td class=""><span>${item.FullName}</span></td>` +
                            `<td class=""><span>${item.Phone}</span></td>` +
                            `<td class=""><span>${item.Address || ''}</span></td>` +
                            `<td class="text-right"><span>${item.TotalAmoutTxt || ''}</span></td>` +
                            `<td class=""><span>${GetStatus(item.Status)}</span></td>` +
                            `<td class=""><span>${GetPaymentStatus(item.PaymentStatus)}</span></td>` +
                            `<td class=""><span>${item.CreatedTimeTxt || ''}</span></td>` +
                            //`<td class="text-center">${htmlShow}</td>` +
                            `</tr>`;
                        startIndex++;
                    }
                    $tableBody.html(html);

                    $pagination.Set($footer, res.Body.Pagination, $page.GetList);

                    $page.ViewClick();
                }
                else {

                }
            })
        },
        ViewClick: () => {
            $page.Self.find('.btnEdit').off('click').on('click', function () {
                let id = $(this).data('id');

                var getResponse = AjaxConfigHelper.SendRequestToServer(`/${$router}/GetItem`, "GET", { 'id': id });
                getResponse.then((res) => {
                    if (res.IsOk) {
                        let data = res.Body.Data || {};
                        for (let prop in data) {
                            if (prop == 'Enable') {
                                $modal.find(`[name=${prop}]`).prop('checked', data[prop] == 1);
                                continue;
                            }
                            if (prop.indexOf('Time') > -1 || prop.indexOf('Ngay') > -1) {
                                data[prop] = formatDateFrom_StringServer(data[prop]);
                            }
                            $modal.find(`[name=${prop}]`).val(data[prop]);
                        }

                        $page.DrawTableItem(data.Items);
                        $modal.modal('show');
                    }
                    else {

                    }
                })
            })
        },
        DrawTableItem: (items) => {
            const $tableItems = $ChiTiet.Self.find('#tbl-items');

            let html = '';
            if (items.length == 0) {
                html = `<tr><td class="text-center" colspan="5"><span>Không có bản ghi</span></td></tr>`;
                $tableItems.find('tbody').html(html);
                return false;
            }
            for (let i = 0; i < items.length; i++) {
                let item = items[i];
                html += `<tr>
                            <td class="text-center">${(i+1)}</td>
                            <td class="">${item.ProductName}</td>
                            <td class="text-right">${item.Price}</td>
                            <td class="text-right">${item.Quantity}</td>
                            <td class="text-right">${item.Price * item.Quantity}</td>
                        </tr>`
            }

            $tableItems.find('tbody').html(html);
        }
    };

    var $ChiTiet = {
        init: function () {
            this.Save();
            this.ResetForm();
        },
        Self: $('#CHITIET'),
        ResetForm: () => {
            $modal.on('hidden.modal.bs', () => {
                $form.find('input').val('');
                $form.find('input:checkbox').prop('checked', true);
                $form.find('select').find('option:first-child').prop('selected', true);
                $form.find('.error').removeClass('error');
            })
        },
        GetDataInput: () => {
            if (!$form.valid()) {
                return false;
            }

            let data = GetFormDataToObject($form);
            return data;
        },
        Save: () => {
            $modal.find('#btn-save').off('click').on('click', () => {
                let data = $ChiTiet.GetDataInput();
                if (!data)
                    return false;
                let action = 'Update';
                var getResponse = AjaxConfigHelper.SendRequestToServer(`/${$router}/${action}`, "POST", data);
                getResponse.then((res) => {
                    if (res.IsOk) {
                        ToastSuccess('Cập nhật thành công');
                        $page.GetList(1);
                        $modal.modal('hide');
                    }
                    else {

                    }
                })
            })
        }
    };


    $page.init();
    $ChiTiet.init();

    function GetStatus(status) {
        return $header.find(`[name="TrangThai"]`).find(`option[value="${status}"]`).html();
    }
    function GetPaymentStatus(status) {
        return $header.find(`[name="TrangThaiThanhToan"]`).find(`option[value="${status}"]`).html();
    }
});